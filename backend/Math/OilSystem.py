...
# jsonPath = "C:/Users/user/Desktop/Тест 4/Тестовый/My project_Data/StreamingAssets/Buffer.json"
...
class OilSystem:
    ...
    def update(self, command_main_run, command_main_stop, command_reserve_run, command_reserve_stop, dt: float):
        """ обновление состояния маслосистемы за шаг dt, время измеряется в сек """
        ...
        # 4. Коррекция вязкости (экспоненциальная зависимость), также зависит от темпы
        k = 0.03  # коэффициент температурной зависимости
        self.viscosity = self.nominal_viscosity * math.exp(-k * (self.temperature - 40))
        self.viscosity = max(10, min(self.viscosity, 100))  # ограничение диапазона
    ...
    @property # используется для обращения к параметрам как к обычным полям, а не функциям
    def oil_pressure(self): # передача давления на сервак
        return self.pressure
        
    @property
    def oil_pressure_ok(self):
        return self.pressure_ok
    ...
    @property
    def oil_temperature(self): # передача температуры на сервак
